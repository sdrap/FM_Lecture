...
!!! example "The Piecewise Linear Function"
    ...
    \begin{equation}
        R(L) 
            ...
            = V@R_{\alpha}(L) + \frac{1}{\alpha} E\left[ \left(L- 
            V@R_{\alpha}(L)\right)^+ \right].
    \end{equation}
    $$
...
!!! proposition
    ...
    \[
      ES_{\alpha}(L) = \frac{1}{\alpha}\int_{0}^\alpha V@R_{s}(L)ds 
        = \inf \left\{ m +\frac{1}{\alpha}E[(L-m)^+] \colon m \in \mathbb{R} \right\} 
        = V@R_{\alpha}(L) + \frac{1}{\alpha}E\left[ \left( L-
            V@R_{\alpha}(L) \right)^+ \right].
    \]
...
To transition to ES, they only need to compute the additional term \( E[(L-
V@R_{\alpha}(L))^+]/\alpha \), which is computationally efficient (either analytically or via Monte Carlo methods).
